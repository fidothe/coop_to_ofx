...
describe CoopScraper::SavingsAccount do
  def fixture_path(fixture_file_name)
    full_fixture_path('savings_account', fixture_file_name)
  end
  
  describe "parsing the html components" do
    def fixture_doc(name = 'savings_account_fixture.html')
    ...
    it "should be able to extract the statement date" do
      CoopScraper::SavingsAccount.extract_statement_date(fixture_doc).should == Time.utc('2008', '10', '3')
    end
    
    it "should be able to extract the account number" do
      CoopScraper::SavingsAccount.extract_account_number(fixture_doc).should == "12341234"
    end
    
    it "should be able to extract the sort code" do
      CoopScraper::SavingsAccount.extract_sort_code(fixture_doc).should == "089273"
    end
    
    describe "transactions" do
      it "should find the correct number of transactions" do
        CoopScraper::SavingsAccount.extract_transactions(fixture_doc).size.should == 7
      end
      
      it "should create OFX::Statement::Transaction objects for the transactions" do
        CoopScraper::SavingsAccount.extract_transactions(fixture_doc).first.should be_instance_of(OFX::Statement::Transaction)
      end
      
      describe "processing a normal transaction" do
        before(:all) do
          transactions = CoopScraper::SavingsAccount.extract_transactions(normal_transaction_fixture_doc)
      ...
      describe "processing a transaction where money was put in" do
        before(:all) do
          transactions = CoopScraper::SavingsAccount.extract_transactions(payment_in_transaction_fixture_doc)
          transactions.size.should == 1
          @transaction = transactions.first
        end
      ...
      describe "processing a statement where no transactions occurred" do
        before(:each) do
          @transactions = CoopScraper::SavingsAccount.extract_transactions(no_transactions_fixture_doc)
        end
      ...
      describe "processing transactions with non-default transaction types" do
        it "should be able to pull out a debit interest transaction" do
          transactions = CoopScraper::SavingsAccount.
    ...
    it "should be able to extract the closing balance" do
      CoopScraper::SavingsAccount.extract_closing_balance(fixture_doc).should == "219.92"
    end
    
    it "should be able to extract the statement start date" do
      CoopScraper::SavingsAccount.extract_statement_start_date(fixture_doc).should == Time.utc('2008', '9', '29')
    end
    
    describe "generating the statement" do
      it "should create a statement object okay" do
        fixture = open(fixture_path('savings_account_fixture.html'))
        mock_statement = mock('Statement')
        OFX::Statement::SavingsAccount.expects(:new).returns(mock_statement)
        ...
        CoopScraper::SavingsAccount.generate_statement(fixture, Time.utc('2009', '3', '6'))
      end
    end
  end
end
